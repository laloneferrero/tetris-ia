...
- **Input**: one `keydown` listener. `P` toggles pause always; other keys are ignored while paused or game over. Horizontal moves are inlined; drops/rotation delegate to helpers. `ArrowUp` and `KeyX` both rotate CW. `Space` calls `e.preventDefault()` (suppress page scroll). `updateHUD()` runs at the end of every keydown, plus inside `softDrop`/`clearLines`; `hardDrop` relies on that trailing call. The keydown listener is a no-op until `started` is set. `restartBtn` click → `init()`; `playBtn` (start screen) → `init()`; `init()` is the sole reset path. At load, `showStartScreen()` runs instead of `init()` — the game starts only when the player presses "Jugar".
- **Overlay**: single `#overlay` element reused for both PAUSA and GAME OVER, toggled via the `hidden` class. `#overlay-gameover-extra` inside it (name input + high-score table + reset button) is shown only for GAME OVER, hidden for PAUSA. `#start-screen` is a separate overlay shown at load and never during play.
- **Records (localStorage)**: `tetris.highscores` (JSON array of `{name, score, lines, combo, date}`, sorted desc, top 5), `tetris.bestCombo`, `tetris.maxLines`. All access wrapped in try/catch. `combo` is a module var: `clearLines()` increments it when `cleared > 0` (tracking `bestComboThisGame`), resets to 0 when a lock clears nothing. `endGame()` persists aggregates and offers to save the score if it makes the top 5.
